...
CWBool CWAssembleMsgElemWTPMAC(CWProtocolMessage *msgPtr) 
{
    ...
    for (i = 0; i < 6; i++) {
    	CWProtocolStore8(msgPtr, mac[i]); 
    }
	
	return CWAssembleMsgElem(msgPtr, CW_MSG_ELEMENT_WTP_MAC_CW_TYPE);
}
...
//		CWDebugLog("AC IPv4 List (%d): %d", i+1, (valPtr->ACIPv4List)[i]);
		...
		addr.sin_port = 1024;
		CWUseSockNtop(&addr, CWDebugLog_D(str););
	}
...
CWBool CWParseACIPv6List(CWProtocolMessage *msgPtr, int len, ACIPv6ListValues *valPtr) 
{
	...
	for(i = 0; i < valPtr->ACIPv6ListCount; i++) {
		...
		addr.sin6_port = htons(g_capwapc_config.ctrl_port);
...
CWBool CWParseAddStation(CWProtocolMessage *msgPtr, int len) 
{
	...
	CWDebugLog_F("STATION'S MAC ADDRESS TO FORWARD TRAFFIC: %02X:%02X:%02X:%02X:%02X:%02X",  
...
CWBool CWParseCWControlIPv4Addresses(CWProtocolMessage *msgPtr, int len, CWProtocolIPv4NetworkInterface *valPtr) {
	...
	valPtr->addr.sin_port = htons(g_capwapc_config.ctrl_port);
	
	CWUseSockNtop((&(valPtr->addr)), CWDebugLog_F("Interface Address: %s", str););
...
CWBool CWParseCWControlIPv6Addresses(CWProtocolMessage *msgPtr, int len, CWProtocolIPv6NetworkInterface *valPtr) {
	...
	valPtr->addr.sin6_port = htons(g_capwapc_config.ctrl_port);
