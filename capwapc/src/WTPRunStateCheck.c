...
void CWWTPRetransmitTimerExpiredHandler(CWTimerArg arg, CWTimerID id)
{
	...
	if(gPendingRequestMsgs[arg].retransmission == g_capwapc_config.max_retran) {
...
void CWWTPRetransmitTimerExpiredHandler(CWTimerArg hdl_arg)
{
    ...
    lock_pendingbox();
	if(gPendingRequestMsgs[index].retransmission == g_capwapc_config.max_retran) {
	...
	CWDebugLog_D("Retransmission Count increases to %d", gPendingRequestMsgs[index].retransmission);
    ...
    unlock_pendingbox();
    /* don't free the hdl_arg, it is gPendingRequestMsgs[index].timer_arg, and 
     * will be free while CWResetPendingMsgBox
     */
//	CW_FREE_OBJECT(hdl_arg);
	return;
}
