...
CWStateTransition CWWTPEnterDataCheck() {
	...
	if(!CWErr(CWWTPSendAcknowledgedPacket(seqNum, 
					      ...
					      NULL))) {
        CWLog("Date check handled failed, will enter RESET state.");
		return CW_ENTER_RESET;
	}
...
CWBool CWAssembleChangeStateEventRequest(CWProtocolMessage **messagesPtr,
    ...
    CWDebugLog_F("Assembling Change State Event Request...");
    ...
    CWDebugLog_F("Change State Event Request Assembled");
	return CW_TRUE;
}
...
CWBool CWParseChangeStateEventResponseMessage(char *msg,
    ...
    CWDebugLog_F("Parsing Change State Event Response...");
	...
	if(controlVal.messageTypeValue != CW_MSG_TYPE_VALUE_CHANGE_STATE_EVENT_RESPONSE) {
        ...
        sprintf(errinfo, "Message(%d) is not Change State Event Response as Expected", 
    ...
    CWDebugLog_F("Change State Event Response Parsed");
	return CW_TRUE;
}
...
CWBool CWSaveChangeStateEventResponseMessage (void *changeStateEventResp)
{
    ...
    CWDebugLog_F("Saving Change State Event Response...");
	CWDebugLog("Change State Event Response Saved");
	CWDebugLog_F("Change State Event Response Saved");
	return CW_TRUE;
}
