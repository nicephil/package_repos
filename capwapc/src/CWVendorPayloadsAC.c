...
CWBool CWAssembleWTPVendorPayloadUCI(CWProtocolMessage *msgPtr) {
	...
	CWDebugLog_F("Assembling Protocol Configuration Update Request [VENDOR CASE]...");
	...
	CWDebugLog_F("Assembling Protocol Configuration Update Request [VENDOR CASE]: Message Assembled.");
...
CWBool CWAssembleWTPVendorPayloadWUM(CWProtocolMessage *msgPtr) {
	...
	switch (valuesPtr->vendorPayloadType){
			case CW_MSG_ELEMENT_VENDOR_SPEC_PAYLOAD_WUM:
				...
				break;
				default:
					CWDebugLog_E("Error! unknown WUM message type!!!");
					return CW_FALSE;
				}
