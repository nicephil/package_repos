...
CWThreadMutex g_pendingbox_mutex;

CWBool create_pendingbox_mutex(void)
{
    return CWCreateThreadMutex(&g_pendingbox_mutex);
}

CWBool lock_pendingbox(void)
{
    return CWThreadMutexLock(&g_pendingbox_mutex);
}

void unlock_pendingbox(void)
{
    CWThreadMutexUnlock(&g_pendingbox_mutex);
}
...
CWBool CWUpdatePendingMsgBox(CWPendingRequestMessage *pendingRequestMsgs, 
	...
	if((pendingRequestMsgs->timer = timer_add(timer_sec, 0, timer_hdl, timer_arg)) < 0) {
		return CW_FALSE;
	}
