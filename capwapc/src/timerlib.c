...
static void *cronometer(void *arg) {
    ...
    openlog("capwap", LOG_PID, LOG_LOCAL3);

    syslog(LOG_DEBUG, "Timer task pid %d.", getpid());
    
	sigemptyset(&mask);
	sigaddset(&mask, SIG_TO_WAIT);
    sigaddset(&mask, SIG_TO_INFORM);
	...
	while(1) {
		...
		sig_expect = sig;
		sig = 0;
		if (sig_expect == SIG_TO_INFORM) {
			/* zjye: maybe add more shorter timer, need to restart */
			syslog(LOG_DEBUG, "Add new shorter timer, stop pre timer.");
			timer_stop();
			continue;             
		}
		else if (sig_expect != SIG_TO_WAIT) {
			syslog(LOG_INFO, "Sigwait acquired a unexpected signal %d.", sig_expect);
			continue;
		}
		/* 
...
int timer_add(long sec, long usec, void(*hndlr)(void *), void *hndlr_arg) {
	...
	if (tmp->prev == NULL) {
		...
		app->in_use = 1;
		tmp->in_use = 0;
        /* zjye:in different theread, here start timer can do nothing, need to inform ticker thread
         * to restart timer
         */
		// timer_start(&(timerq.first->timeout));
		pthread_mutex_unlock(&timerq.mutex);
        pthread_kill(timerq.ticker, SIG_TO_INFORM);
        return id;
	}
	else {
		app->prev = tmp->prev;
...
int timer_rem(int id, void(* free_arg)(void *)) {
