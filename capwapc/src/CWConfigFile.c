...
CWBool CWParseTheFile(CWBool isCount) {
			...
			if(!strncmp(line, 
				...
				switch(gConfigValues[i].type) {
					...
					case CW_STRING_ARRAY:
						#ifdef CW_DEBUGGING
							CWDebugLog_F("*** Parsing String Array... *** \n");
						#endif
						j = 0;
						CW_FREE_OBJECT(line);
						while((line = CWGetCommand(gCWConfigFile)) != NULL && !strncmp(line, gConfigValues[i].endCode, strlen(gConfigValues[i].endCode))) {
							#ifdef CW_DEBUGGING
								CWDebugLog_F("*** Parsing String (%s) *** \n", line);
							#endif
                            myLine = line + strlen(gConfigValues[i].endCode);
	...
	CWDebugLog_F("*** Config File Parsed ***");
