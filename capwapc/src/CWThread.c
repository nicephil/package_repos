...
void CWDestroyThread(CWThread thread) {
...
CWBool CWThreadTimedSemWait(CWThreadTimedSem *semPtr, time_t sec, time_t nsec) {
	...
	timeout.tv_nsec = nsec;
		...
		while(sem_timedwait(semPtr, &timeout) < 0 ) {
			...
			if(errno == EINTR) { continue;}
				...
				CWDebugLog_F("sem_timedwait expired");
				return CWErrorRaise(CW_ERROR_TIME_EXPIRED, NULL);
			} else {
				CWErrorRaiseSystemError(CW_ERROR_GENERAL);
			}
	...
	CWDebugLog_F("Timed Sem Wait Before Select");
	while((r = select(((*semPtr)[0])+1, &fset, NULL, NULL, &timeout)) <= 0) {
		CWDebugLog_E("Timed Sem Wait Select error");
		if(r == 0) {
			CWDebugLog_F("Timed Sem Wait Timeout");
			return CWErrorRaise(CW_ERROR_TIME_EXPIRED, NULL);
		} else if(errno == EINTR) {
			timeout.tv_sec = sec;
			timeout.tv_usec = nsec / 1000;
	...
	CWDebugLog_F("Timed Sem Wait After Select");
	...
	CWDebugLog_F("Timed Sem Wait Before Select 2");
	while((r = select(((*semPtr)[0])+1, &fset, NULL, NULL, &timeout)) <= 0) {
		CWDebugLog_E("Timed Sem Wait Select error 2");
		if(r == 0) {
			CWDebugLog_F("Timed Sem Wait Timeout 2");
			return CWErrorRaise(CW_ERROR_TIME_EXPIRED, NULL);
		} else if(errno == EINTR) {
			timeout.tv_sec = 2;
			timeout.tv_usec = 0;
	...
	CWDebugLog_F("Timed Sem Wait After Select 2");
	...
	CWDebugLog_F("End of Timed Sem Wait");
...
CWBool CWThreadTimedSemPost(CWThreadTimedSem *semPtr) {
	...
	CWDebugLog_F("Timed Sem Post Before Select");
	while((r = select(((*semPtr)[1])+1, &fset, NULL, NULL, &timeout)) <= 0) {
		CWDebugLog_E("Timed Sem Post Select Error");
		if(r == 0) { // timeout, server is not responding
			...
			CWDebugLog_F("Timed Sem Post Timeout");
	...
	CWDebugLog_F("Timed Sem Post After Select");
...
CWBool CWThreadCreateSpecific(CWThreadSpecific *specPtr, void (*destructor)(void *)) {
	...
	if(pthread_key_create(specPtr, destructor) != 0) {
		CWDebugLog_E("Error pthread key create");
		return CW_FALSE;
	}
...
void CWExitThread() {
    CWLog("*** Exit Thread(%d:%ld) ***.", getpid(), pthread_self());
...
void CWHandleTimer(CWTimerArg arg) {
	...
	CWThreadSendSignal(requestedThreadPtr, signalToRaise);
 	CWDebugLog_F("Timer Expired, Sent Signal(%d) to Thread: %d", (int)signalToRaise, (int)requestedThreadPtr);
...
CWBool CWTimerRequest(int sec, CWThread *threadPtr, CWTimerID *idPtr, int signalToRaise) {
	...
	CWDebugLog_F("Timer Request: thread(%d), signal(%d)", (int)*(arg->requestedThreadPtr), (int)arg->signalToRaise);
