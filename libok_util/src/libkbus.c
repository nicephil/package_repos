                                    ...
                                    int      flags)
{
  ...
  if (rv < 0)
    ...
    flags = fcntl(rv, F_GETFD);
    if (flags != -1) {
        flags |= FD_CLOEXEC;
        fcntl(rv, F_SETFD, flags);
    }
    return rv;
  }
}
...
/*
 * NOTICE: 
 *  reply message's name will be cloned, so these two message 
 *  has no correlation now!
 *
 *  User MUST FREE this name of reply message too
 */
                                    ...
                                    uint32_t flags)
{
  ...
  name2 = malloc(in_reply_to->name_len);
  if (name2 == NULL) {
      return -ENOMEM;
  }
  memcpy(name2, name, in_reply_to->name_len);
  rv = kbus_msg_create(msg, name2, in_reply_to->name_len, data, data_len, flags);
  if (rv) return rv;
                                     ...
                                     char                 **name)
{
  ...
  this_name[event_data->name_len] = 0;
