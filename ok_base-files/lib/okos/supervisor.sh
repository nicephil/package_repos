...
do
    ...
    [ -z "$capwap_pid" -o "$capwap_count" -gt "1" ] && {
        logger -t supervisor -p 3 "CAPWAP is exit abnormally, restart it !!!"
    ...
    [ -z "$wifidog_pid" -o "$wifidog_count" -gt "1" ] && {
        logger -t supervisor -p 3 "WIFIDog is exit abnormally, restart it !!!"
    ...
    [ -z "$clientevent_pid" -o "$clientevent_count" -gt "1" ] && {
        logger -t supervisor -p 3 "CLIENTEVENT is exit abnormally, restart it !!!"
    ...
    do
        ...
        [ -z "$hostapd_cli_pid" -o "$hostapd_cli_count" -gt "1" ] && {
            logger -t supervisor -p 3 "HOSTAPD_CLI is exit abnormally, restart it !!!"
