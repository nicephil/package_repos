...
_ip=$(ifconfig eth1 | awk -F'[ :]+' '/inet addr/{print $4}')
_mask=$(ifconfig eth1 | awk -F':' '/Mask/{print $4}')
_vendor=${_model%%_*}
...
do
    ...
    aria2c -x 5 --min-split-size=2M --file-allocation=none -c  "$main_image_server_url" "$IMAGE_URL" $other_image_server_urls -d "/" -o ${OKOS_FILE} 2>&1 | logger -t 'handle_cloud'
