...
//                        CWDebugLog("Found free timer"); 
        ...
        if (t == &timerq[CW_TIMER_MAX]) {
			CWDebugLog_E("Free timer not found"); 
			enable_interrupts()
...
//		CWDebugLog("Longer Timer (%d)\n", timeout);
	...
	CWDebugLog_F("timer set, index:%d, timeout:%d", *idPtr, timerq[*idPtr].timeout);
        enable_interrupts(); 
        return(TRUE); 
} 
...
void clk_isr() { 
	CWDebugLog_F("Signal Timer Expired received");
        /* the following condition could be set up, 
        if the interrupt was delivered while we were in cantim, cancelling 
        the last timer */ 
		//printf("\n\n\n%d\n\n", pthread_self());
	CWDebugLog_F("Subtract %d from timers", current_time() - timer_set_timestamp);
