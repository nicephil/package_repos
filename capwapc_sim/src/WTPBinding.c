...
CWBool CWWTPInitBinding(int radioIndex)
{
	...
	if (sock < 0) 
	{
		CWDebugLog_E("Error Creating Socket for ioctl"); 
		return CW_FALSE;
	}
...
CWBool CWManageOFDMValues(CWBindingConfigurationUpdateRequestValuesOFDM *ofdmValues, CWProtocolResultCode *resultCode) {
	...
	*resultCode = CW_PROTOCOL_SUCCESS;
	...
	if ((sendSock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) ==-1 ) {
		CWDebugLog_E("[FreqAnalyzer]: Error on creation of socket.");
		return CWErrorRaise(CW_ERROR_GENERAL, NULL);
	}
	...
	if ( inet_aton(FREQ_SERVER_ADDR, &serv_addr.sin_addr)==0 ) {
	  CWDebugLog_E("[CWManageOFDMValue]: Error on aton function.");
	  close(sendSock);
	  return CWErrorRaise(CW_ERROR_GENERAL, NULL);
	}
	...
	if ( sendto(sendSock, radioValues, sizeof(OFDMControlValues), 0, (struct sockaddr *) &serv_addr, slen) < 0 ) {
		CWDebugLog_E("[CWManageOFDMValue]: Error on sendto function.");
		close(sendSock);
		return CWErrorRaise(CW_ERROR_GENERAL, NULL);
	}
...
CWBool CWBindingParseConfigurationUpdateRequest (char *msg, int len, void **valuesPtr) 
{
	...
	CWDebugLog("Parsing Binding Configuration Update Request...");
    CWDebugLog_F("Parsing Binding Configuration Update Request...");
	...
	CWDebugLog("Binding Configure Update Request Parsed");
    CWDebugLog_F("Binding Configure Update Request Parsed");
...
CWBool CWBindingParseConfigureResponse (char *msg, int len, void **valuesPtr){
	...
	CWDebugLog("Parsing Binding Configuration Request...");
    CWDebugLog_F("Parsing Binding Configuration Request...");
	...
	*valuesPtr = (void *)auxBindingPtr;
	...
	while(completeMsg.offset < len) {
		...
		CWDebugLog_F("Parsing Message Element: %d, elemLen: %d", elemType, elemLen);
	...
	CWDebugLog("Binding Configuration Request Parsed");
	CWDebugLog_F("Binding Configuration Request Parsed");
	return CW_TRUE;
}
