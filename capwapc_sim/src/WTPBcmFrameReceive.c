...
CW_THREAD_RETURN_TYPE CWWTPReceiveFrame(void *arg)
{
	...
	if(!get_mac(wl0_mac_address)) {
		...
		CWDebugLog_E("THR FRAME: Failure to get interface mac address");
		EXIT_THREAD
    	}
	...
	if (i != WLC_IOCTL_MAGIC) {
		...
		CWDebugLog_E("THR FRAME: Wireless magic not correct, not querying wl for info");
		EXIT_THREAD
	}
	else {
	...
	if (!handle) {
		...
		CWDebugLog_E("THR FRAME: Failure to open pcap!");
		EXIT_THREAD
    	}
