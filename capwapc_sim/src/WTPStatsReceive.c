...
CW_THREAD_RETURN_TYPE CWWTPReceiveStats(void *arg)
{
	...
	{
                CWDebugLog_E("THR STATS: Error creating socket");
		CWExitThread();
        }
	...
	{
                CWDebugLog_E("THR STATS: Error binding socket");
		CWExitThread();
        }
...
/*      Receive data */
	CW_REPEAT_FOREVER 
	{
		rlen = recvfrom(sock, buffer,PACKET_SIZE, 0, (struct sockaddr *)&from,(socklen_t *)&fromlen);
        	if (rlen == -1) 
		{
        		CWDebugLog_E("THR STATS: Error receiving from unix socket");
			CWExitThread();   
        	} 
		else 
		{	completeMsgPtr = NULL;
			...
			if(!create_data_Frame(&data,buffer,rlen)){
								CWDebugLog_E("Error extracting a data stats frame");
								CWExitThread();
							};
