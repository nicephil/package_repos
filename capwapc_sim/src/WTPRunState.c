...
CWStateTransition CWWTPEnterRun() {
    ...
    if (devctrl_thread != -1)
        CWDestroyThread(devctrl_thread);
		...
		if ((CWGetCountElementFromSafeList(gPacketReceiveList) == 0) && (CWGetCountElementFromSafeList(gFrameList) == 0)) {
			...
			if (!CWErr(CWWaitThreadConditionTimeout(&gInterfaceWait, &gInterfaceMutex, &timenow))) {
				...
				if (CWErrorGetLastErrorCode() == CW_ERROR_TIME_EXPIRED)	{
					...
					CWDebugLog("No Message from AC for a long time... restart Discovery State");
                    CWDebugLog_E("No Message from AC for a long time... restart Discovery State");
					break;
				}
				continue;
			}
...
//				CWLog("Echo Response received");
				if (count++ % ECHO_RATELIMIT_TIMES == 0) {
					if (count == 1) {
						CWLog("Echo Response received");
			...
			default:
				 ...
				 */
				CWDebugLog("--> Not valid Request (%d) in Run State... we send a failure Response",
				    controlVal.messageTypeValue);
				/*Update 2009:
					check to see if a time-out on session occur...
					In case it happens it should go back to CW_ENTER_RESET*/
		...
		switch(controlVal.messageTypeValue) 
		{
			...
			default:
				 ...
				 */
				CWDebugLog("can't recognize the received Response :%d", controlVal.messageTypeValue);
...
void CWWTPHeartBeatTimerExpiredHandler(void *arg) {
	...
	if (count++ % ECHO_RATELIMIT_TIMES == 0) {
		if (count == 1) {
			CWDebugLog("WTP HeartBeat Timer Expired... send an ECHO Request");
            CWDebugLog_F("WTP HeartBeat Timer Expired... send an ECHO Request");
		}
		else {
			CWDebugLog("WTP HeartBeat Timer Expired... send an ECHO Request (compressed %d times)", ECHO_RATELIMIT_TIMES);
