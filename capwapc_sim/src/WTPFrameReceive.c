...
/*	CWDebugLog("Address 1:");
	...
	if (ok==1) {CWDebugLog_F("MAC Address test: OK\n");}
	else {CWDebugLog_E("MAC Address test: Failed\n");}
...
CW_THREAD_RETURN_TYPE CWWTPReceiveFrame(void *arg)
{
	...
	{
		CWDebugLog_E("THR FRAME: Error creating socket");
		CWExitThread();
	}
	...
	{
		CWDebugLog_E("THR FRAME: Error binding socket");
		CWExitThread();
	}
	
	if (!getMacAddr(sock, gInterfaceName, macAddr))
	{
		CWDebugLog_E("THR FRAME: Ioctl error");
		EXIT_THREAD
	}
	...
	{
		CWDebugLog_E("THR FRAME: Error ioctl");
		EXIT_THREAD
	}
	...
	{
		CWDebugLog_E("THR FRAME: Error attaching filter");
		EXIT_THREAD
	}
#endif
				...
				if(type == (unsigned char) MANAGEMENT_TYPE) 		//Management Frame
				{
					...
					switch (subtype)
					{
						case ASSOCIATION_REQUEST_SUBTYPE: 
						{
							...
							{
								CWDebugLog_E("THR FRAME: Error extracting a frame");
								EXIT_THREAD
							}
