...
static bool
cfg_attr_to_blob(struct blob_buf *b, const char *str,
...
static void
cfg_array_to_blob(struct blob_buf *b, struct cfg_option *o,
		  enum blobmsg_type type)
{
	...
	if (o->type == CFG_TYPE_LIST) {
		cfg_foreach_element(&o->v.list, e) {
			cfg_attr_to_blob(b, e->name, NULL, type);
		}
		return;
	}
	...
	while ((word = strsep(&next, " \t")) != NULL) {
		...
		cfg_attr_to_blob(b, word, NULL, type);
	}
...
static int
__cfg_element_to_blob(struct blob_buf *b, struct cfg_element *e,
		      const struct cfg_blob_param_list *p)
{
	...
	for (i = 0; i < p->n_params; i++) {
		...
		if (attr->type == BLOBMSG_TYPE_ARRAY) {
			...
			cfg_array_to_blob(b, o, element_type);
		...
		ret += cfg_attr_to_blob(b, o->v.string, attr->name, attr->type);
	}
	return ret;
}

static int
__cfg_to_blob(struct blob_buf *b, struct cfg_section *s,
	      const struct cfg_blob_param_list *p)
{
	struct cfg_element *e;
	int ret = 0;
	...
	cfg_foreach_element(&s->options, e)
		ret += __cfg_element_to_blob(b, e, p);

	return ret;
}

int
cfg_to_blob(struct blob_buf *b, struct cfg_section *s,
	    const struct cfg_blob_param_list *p)
{
	int ret = 0;
	int i;

	ret += __cfg_to_blob(b, s, p);
	for (i = 0; i < p->n_next; i++)
		ret += cfg_to_blob(b, s, p->next[i]);

	return ret;
}

bool
cfg_blob_diff(struct blob_attr **tb1, struct blob_attr **tb2,
	      const struct cfg_blob_param_list *config, unsigned long *diff)
{
	bool ret = false;
	int i;
...
static bool
__cfg_blob_check_equal(struct blob_attr *c1, struct blob_attr *c2,
		       const struct cfg_blob_param_list *config)
{
	...
	return !cfg_blob_diff(tb1, tb2, config, NULL);
}

bool
cfg_blob_check_equal(struct blob_attr *c1, struct blob_attr *c2,
		     const struct cfg_blob_param_list *config)
{
	int i;

	if (!__cfg_blob_check_equal(c1, c2, config))
		return false;

	for (i = 0; i < config->n_next; i++) {
		if (!__cfg_blob_check_equal(c1, c2, config->next[i]))
			return false;
	}
